...
- **`launch_chrome` returns `(browser, ctx, page)`, not `pages[0]`.** Chrome launched via `open -n -a` against a persistent `--user-data-dir` surfaces multiple targets in `ctx.pages` — a fresh `chrome://newtab/` (the only one with a real OS window), session-restored tabs from prior runs (no window — restored as deserialized targets without compositor surface), and auxiliary `chrome://omnibox-popup.top-chrome/` UI tabs (also windowless). `ctx.pages[0]` deterministically lands on a windowless target; DOM/CDP/input keep working but `Page.captureScreenshot` hangs forever because there's no paint frame. `_pick_windowed_page` probes each via `Browser.getWindowForTarget` and returns the first windowed one. **Don't try to close the windowless ones** — `page.close()` on `chrome://omnibox-popup` hangs (these are auxiliary UI surfaces, not real tabs Playwright can close); leave them, they're harmless background orphans torn down at `browser.close()`. Symptom of regression: every `screenshot_skipped` line in worker.stderr with `"waiting for fonts to load... fonts loaded"` followed by timeout, plus the user can't see the chatgpt tab in their VNC view.
